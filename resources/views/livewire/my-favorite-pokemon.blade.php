<div>
    ...
    <div class="grid gap-6 lg:grid-cols-4 lg:gap-8 my-pokemon">
    ...
    <div class="grid gap-6 lg:grid-cols-4 lg:gap-8 pokemon-list">
        @foreach ($pokemons as $pokemon)
            <div class="bg-white dark:bg-zinc-900 rounded-lg shadow-[0px 4px 34px rgba(0,0,0,0.06)] pokemon">
                ...
                />
                <div class="p-6 text-center">
                    <h2 class="text-xl font-semibold name">{{ $pokemon->name }}</h2>
                    <p class="text-sm mt-3">{{ $pokemon->description }}</p>

                    <button
                        wire:click="addPokemon({{ $pokemon->id }})"
                        class="mt-4 bg-red-400 text-white rounded-lg px-4 py-2 add-pokemon"
                    >
